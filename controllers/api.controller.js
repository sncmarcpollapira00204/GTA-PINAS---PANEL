...
const currentTranscriptIds = Object.values(config.transcriptChannels || {}).filter(Boolean).map(String);
...
function sanitizeTranscriptHtml(input) {
  ...
  // Preserve transcript markup and styling while removing active content.
  html = html
...
exports.getDashboardStats = async (req, res) => {
  try {
    ...
    if (!dashboardPromise) {
      ...
      `, [configuredStaffCount, currentTranscriptIds]).finally(() => {
    ...
    const payload = {
      ...
      staffOnline: stats.staff_registered || 0,
    };

    if (dashboardCache.payload?.ticketsVersion !== payload.ticketsVersion) {
      ticketsCache = { expiresAt: 0, rows: null };
    }
...
exports.getTickets = async (req, res) => {
  try {
    ...
    if (!normalizedStatus && ticketsCache.rows && ticketsCache.expiresAt > Date.now()) {
      return res.json(ticketsCache.rows);
    }
    ...
    values.push(currentTranscriptIds);
    ...
    if (!normalizedStatus) {
      if (!ticketsPromise) {
        ticketsPromise = pool.query(query, values).finally(() => {
          ticketsPromise = null;
        });
      }
      result = await ticketsPromise;
      ticketsCache = {
        expiresAt: Date.now() + TICKETS_CACHE_MS,
        rows: result.rows,
      };
    } else {
...
exports.getStaff = async (req, res) => {
  try {
    const result = await pool.query(`
      SELECT *
      FROM staff
      ORDER BY tickets_handled DESC, username ASC
    `);
