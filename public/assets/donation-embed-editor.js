(() => {
    ...
    function injectStyles() {
        ...
        style.textContent = `
            ...
            #view-donation-embed-editor .dee-link-box{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:8px;align-items:start;margin-bottom:18px;padding-bottom:18px;border-bottom:1px solid var(--border)}
            #view-donation-embed-editor .dee-link-box .dee-field{min-width:0}
            #view-donation-embed-editor .dee-link-box .btn{align-self:end;width:108px;min-width:108px;height:36px;min-height:36px;box-sizing:border-box;padding:0 10px;justify-content:center;align-items:center;white-space:nowrap;margin:0}
            #view-donation-embed-editor .dee-link-box .dee-field input{height:36px}
    ...
    function formatDescription(value) {
        return String(value || '').split('\n').map((line) => {
            ...
            return line ? formatInline(line) : '<br>';
        }).join('\n');
    }
    ...
    function renderPreview() {
        ...
        const image = safeUrl(data.image); const thumbnail = safeUrl(data.thumbnail);
        const preview = document.getElementById('dee-preview'); if (!preview) return;
        preview.innerHTML = `
            <div class="dee-discord-user"><div class="dee-discord-avatar">5A</div><div><strong>GTA Pinas Treasury</strong><span>Today at ${new Date().toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' })}</span></div></div>
            <div class="dee-embed" style="border-left-color:${esc(color)}${thumbnail ? ';' : ''} ${thumbnail ? '' : ''}">
                ${thumbnail ? `<img class="dee-embed-thumbnail" src="${esc(thumbnail)}" alt="">` : ''}
                <div class="dee-embed-main${thumbnail ? ' has-thumbnail' : ''}">
                    ${data.author ? `<div class="dee-embed-author">${esc(data.author)}</div>` : ''}
                    ${data.title ? `<div class="dee-embed-title">${esc(data.title)}</div>` : ''}
                    ${data.description ? `<div class="dee-embed-description">${formatDescription(data.description)}</div>` : '<div class="dee-embed-description" style="color:#949ba4">Start typing to preview your message.</div>'}
                    ${image ? `<img class="dee-embed-image" src="${esc(image)}" alt="">` : ''}
    ...
    function bindLivePreview() {
        ['dee-title','dee-description','dee-color','dee-image','dee-thumbnail','dee-author','dee-footer'].forEach((id) => document.getElementById(id)?.addEventListener('input', renderPreview));
        renderPreview();
    }
    ...
    function bindToolbar() {
        ...
        document.addEventListener('keydown', (event) => {
            ...
            const key = event.key.toLowerCase(); const map = { b: 'bold', i: 'italic', u: 'underline', s: 'strike', e: 'code' }; if (map[key]) { event.preventDefault(); applyFormat(map[key]); }
        });
    }

    function applyFormat(format) {
        const field = document.getElementById('dee-description'); if (!field) return;
        const start = field.selectionStart || 0; const end = field.selectionEnd || 0; const selected = field.value.slice(start, end); if (!selected && format !== 'link') return;
        const wrappers = { bold: ['**','**'], italic: ['*','*'], underline: ['__','__'], strike: ['~~','~~'], code: ['`','`'], spoiler: ['||','||'], quote: ['> ',''] };
        let replacement = selected;
        if (format === 'link') { const url = window.prompt('Enter URL'); if (!url) return; replacement = `[${selected}](${safeUrl(url) || url})`; }
        else if (wrappers[format]) replacement = `${wrappers[format][0]}${selected}${wrappers[format][1]}`;
        field.setRangeText(replacement, start, end, 'select'); field.dispatchEvent(new Event('input', { bubbles: true })); field.focus();
    }

    async function loadChannels() {
        const select = document.getElementById('dee-channel'); if (!select) return;
        try {
            const response = await fetch('/api/donation/channels'); const payload = await response.json(); if (!response.ok) throw new Error(payload?.message || 'Failed to load channels');
            state.channels = Array.isArray(payload.channels) ? payload.channels : []; select.innerHTML = state.channels.length ? state.channels.map((channel) => `<option value="${esc(channel.id)}">${esc(channel.name || channel.id)}</option>`).join('') : '<option value="">No donation channels configured</option>';
    ...
    function fill(data) {
        ...
        const fields = { title: embed.title || '', description: embed.description || '', color: embed.color ? `#${Number(embed.color).toString(16).padStart(6,'0').toUpperCase()}` : '#2563EB', image: embed.image?.url || '', thumbnail: embed.thumbnail?.url || '', author: embed.author?.name || '', footer: embed.footer?.text || '' };
        Object.entries(fields).forEach(([key, value]) => { const input = document.getElementById(`dee-${key}`); if (input) input.value = value; });
        state.preserve = { fields: Array.isArray(embed.fields) ? embed.fields : [], url: embed.url || '', timestamp: embed.timestamp || null };
        renderPreview();
    }

    async function loadExisting() {
        const input = document.getElementById('dee-message-url'); const url = String(input?.value || '').trim();
        if (!url) return setStatus('Paste a Discord message link first.', 'error');
        setStatus('Loading embed...');
        try {
            const response = await fetch(`/api/donation/message?url=${encodeURIComponent(url)}`); const payload = await response.json(); if (!response.ok) throw new Error(payload?.message || 'Failed to load embed');
            fill(payload);
            state.linkedMessageUrl = url;
            const linked = document.getElementById('dee-linked'); if (linked) linked.textContent = payload?.channel?.name ? `Linked: #${payload.channel.name}` : 'Existing message linked';
            document.getElementById('dee-update-button')?.classList.add('visible'); setStatus('Embed loaded. You can now edit and update it.', 'success');
            if (payload?.channel?.id && state.channels.some((channel) => String(channel.id) === String(payload.channel.id))) document.getElementById('dee-channel').value = payload.channel.id;
        } catch (error) { setStatus(error.message, 'error'); }
    }

    async function send() {
        const channelId = document.getElementById('dee-channel')?.value; if (!channelId) return setStatus('Select a donation channel first.', 'error');
        setStatus('Sending embed...');
        try {
            const response = await fetch('/api/donation/embed', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ channelId, embed: formData() }) });
            const payload = await response.json(); if (!response.ok) throw new Error(payload?.message || 'Failed to send embed'); setStatus('Embed sent successfully.', 'success');
        } catch (error) { setStatus(error.message, 'error'); }
    }
    ...
    async function updateExisting() {
        ...
        try {
            const response = await fetch('/api/donation/message', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ messageUrl: state.linkedMessageUrl, embed: formData(), preserve: state.preserve }) });
            const payload = await response.json(); if (!response.ok) throw new Error(payload?.message || 'Failed to update embed'); setStatus('Existing embed updated successfully.', 'success');
        } catch (error) { setStatus(error.message, 'error'); }
    }

    function reset() {
        state.linkedMessageUrl = ''; state.preserve = {};
        ['dee-message-url','dee-title','dee-description','dee-image','dee-thumbnail'].forEach((id) => { const field = document.getElementById(id); if (field) field.value = ''; });
        const color = document.getElementById('dee-color'); if (color) color.value = '#2563EB';
        ['dee-author','dee-footer'].forEach((id) => { const field = document.getElementById(id); if (field) field.value = ''; });
        const linked = document.getElementById('dee-linked'); if (linked) linked.textContent = '';
        document.getElementById('dee-update-button')?.classList.remove('visible'); setStatus(''); renderPreview();
    }

    function openEditor() {
        buildView(); injectStyles(); loadChannels(); const section = document.getElementById('view-donation-embed-editor'); if (section) section.classList.add('active');
        if (window.lucide) window.lucide.createIcons();
    }

    function init() { injectStyles(); buildSidebarItem(); buildView(); openEditor(); }
