...
(() => {
  const RULE_SECTIONS = [
    { type: 'heading', title: 'CITY LAW NO. 2 — ROLEPLAY & CITY RULES', intro: 'All citizens are required to understand and follow the rules below. Ignorance of the rules does not excuse a violation.' },
    ...
    { type: 'heading', title: 'CITY CODE NO. 1 — DISCORD RULES', intro: 'These rules apply to the GTA Pinas Discord community and are part of the overall server ruleset.' },
    ...
    { type: 'heading', title: 'POINT WAR RULES', intro: 'Point War is treated as an OOC competitive event while the event is active.' },
    ...
    { type: 'notice', title: 'FINAL NOTICE', rules: [
  ...
  function renderRules(query = '') {
    ...
    RULE_SECTIONS.forEach((section) => {
      if (section.type === 'heading') {
        html += `<section class="city-rules-block"><div class="city-rules-block-heading"><h2>${escapeHtml(section.title)}</h2><p>${escapeHtml(section.intro)}</p></div><div class="city-rules-section-articles">`;
        return;
      }
  ...
  function ensureNavigation() {
    ...
    view.innerHTML = `
      <div class="city-rules-shell">
        ...
        <div class="city-rules-toolbar">
          ...
          <button type="button" id="city-rules-expand">Show all rules</button>
    ...
    document.getElementById('city-rules-expand').addEventListener('click', () => renderRules(document.getElementById('city-rules-search')?.value || ''));
